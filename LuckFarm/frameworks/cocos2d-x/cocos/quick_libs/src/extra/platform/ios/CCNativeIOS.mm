...
- (void)showActivityIndicator:(UIActivityIndicatorViewStyle)style
{
    ...
    int width = 191, height = 191;
    
    float scale = h / 768.0f;
    
    scale = scale * 0.7;
    
    float rectW = scale * width;
    float rectH = scale * height;
    
    frame = CGRectMake((w - rectW) / 2, (h - rectW) / 2, rectW, rectW);
    
    
    activityIndicatorView_ = [[UIView alloc] initWithFrame:frame];
    
    
    UIImageView  *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0,0,rectW,rectH)];
    imageView.image = [UIImage imageNamed:@"res/loding_cyc"];

    [activityIndicatorView_ addSubview:imageView];
    [imageView release];
